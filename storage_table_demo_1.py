...
resourcegroup_name = 'ssh'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'ssh'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'westus'
...
car.cost = 20000
table_service.insert_entity('itemstable', car)
print('Created entry for Toyota Carolla...')
...
car.cost = 25000
table_service.insert_entity('itemstable', car)
print('Created entry for Honda Accura...')
...
print('Created entry for Coffee Nescafe hazelnut...')
...
coffee.cost = 2.75
table_service.insert_entity('itemstable', coffee)
print('Created entry for Coffee Nescafe light...')
...
coffee.cost = 2.50
table_service.insert_entity('itemstable', coffee)
print('Created entry for Coffee Starbucks...')
...
coffee.cost = 3.50
table_service.insert_entity('itemstable', coffee)
print('Created entry for Coffee Starbucks latte...')
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'carlist'", select='make,model,cost')
for item in items:
    print('BrandName: ' + item.make)
    print('Name: ' + item.model)
    print('Cost: ' + str(item.cost) + '\n')

items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeemenu'", select='brand,flavor,cost')
for item in items:
    print('BrandName: ' + item.brand)
